...
import { eventsUpcoming } from "../../lib/queries";
...
type EventItem = {
  ...
  standardPriceAED?: number; memberPriceAED?: number;
  status?: string; series?: string;
};

export default async function EventsPage() {
  const items = await client.fetch<EventItem[]>(eventsUpcoming);
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-3xl font-semibold">Events at DLIC</h1>
      <p className="mt-2 text-neutral-600">Register → Pay invoice → Get confirmation.</p>
      <div className="mt-8 grid md:grid-cols-3 gap-6">
        {items.map((e) => (
          <article key={e._id} className="rounded-2xl border border-dashed border-neutral-300 bg-neutral-50 p-4">
